...
// NewVideoProvider creates a VideoProvider from a provider name
// ("fal", "gemini", "minimax", "veo").
func NewVideoProvider(providerName, apiKey, model string) (VideoProvider, error) {
	...
	case "minimax":
		return newMinimaxVideoProvider(apiKey, model), nil
	case "veo":
		return newVeoVideoProvider(apiKey, model), nil
	default:
